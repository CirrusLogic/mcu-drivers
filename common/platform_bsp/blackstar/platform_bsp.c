...
#define BSP_LED_SEL_LED_1                           (2)
#define BSP_LED_SEL_LED_2                           (3)
#define BSP_LED_SEL_LED_3                           (4)
#define BSP_LED_TOTAL                               (5)
...
static bool bsp_pb_pressed_flag = false;
...
EXTI_HandleTypeDef exti_sel_gpi_handle, exti_int_handle;
...
static void bsp_exti_sel_gpi_cb(void)
{
...
void HAL_MspInit(void)
{
    ...
    // Configure GPIO pins : SEL_GPI (PA2)
    ...
    HAL_EXTI_SetConfigLine(&exti_sel_gpi_handle, &exti_config);
    HAL_EXTI_RegisterCallback(&exti_sel_gpi_handle, HAL_EXTI_COMMON_CB_ID, &bsp_exti_sel_gpi_cb);

    // Configure GPIO pins : SEL_LED_1, 2, 3 (PB9, PC13, PA3)
    ...
    HAL_NVIC_SetPriority(EXTI2_IRQn, BSP_DUT_INT_PREEMPT_PRIO, 0);
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
    ...
    if (htim->Instance == TIM5)
    {
        ...
        for (uint8_t i = 0; i < BSP_LED_TOTAL; i++)
        {
            // If LED is in fixed mode
            if (led->mode == BSP_LED_MODE_FIXED)
            {
                // Change LED GPIO
                if (led->is_on)
                {
...
uint32_t bsp_set_gpio(uint32_t gpio_id, uint8_t gpio_state)
{
    switch (gpio_id)
    {
        ...
        case BSP_GPIO_ID_INTP_LED3:
            HAL_GPIO_WritePin(GPIOB, GPIO_PIN_9, (GPIO_PinState) gpio_state);
            break;
        ...
        case BSP_GPIO_ID_INTP_LED5:
            HAL_GPIO_WritePin(GPIOA, GPIO_PIN_3, (GPIO_PinState) gpio_state);
            break;

        case BSP_GPIO_ID_GF_GPIO10:
            HAL_GPIO_WritePin(GPIOA, GPIO_PIN_12, (GPIO_PinState) gpio_state);
            break;

        case BSP_GPIO_ID_GF_GPIO11:
            HAL_GPIO_WritePin(GPIOA, GPIO_PIN_8, (GPIO_PinState) gpio_state);
            break;

        case BSP_GPIO_ID_GF_GPIO12:
            HAL_GPIO_WritePin(GPIOA, GPIO_PIN_9, (GPIO_PinState) gpio_state);
            break;

        case BSP_GPIO_ID_GF_GPIO13:
            HAL_GPIO_WritePin(GPIOB, GPIO_PIN_10, (GPIO_PinState) gpio_state);
            break;
