...
void HAL_MspInit(void)
{
    ...
    // Configure GPIO pins : SEL_GPI (PA8)
    ...
    // Configure GPIO pins : SEL_LED_1, 2, 3 (PA9, 10, 11)
    ...
    HAL_NVIC_SetPriority(EXTI9_5_IRQn, BSP_DUT_INT_PREEMPT_PRIO, 0);
...
uint32_t bsp_set_gpio(uint32_t gpio_id, uint8_t gpio_state)
{
    switch (gpio_id)
    {
        ...
        case BSP_GPIO_ID_INTP_LED4:
            HAL_GPIO_WritePin(GPIOA, GPIO_PIN_10, (GPIO_PinState) gpio_state);
            break;

        case BSP_GPIO_ID_INTP_LED5:
            HAL_GPIO_WritePin(GPIOA, GPIO_PIN_11, (GPIO_PinState) gpio_state);
            break;
