...
/**
 * @defgroup CS47L15_STATUS_
 * @brief Return values for all public and most private API calls
 *
 * @{
 */
#define CS47L15_STATUS_OK                               (0)
#define CS47L15_STATUS_FAIL                             (1)
/** @} */

/**
 * @defgroup CS47L15_BUS_TYPE_
 * @brief Types of serial bus to control the CS47L15
 *
 * @see cs47l15_bsp_config_t member bus_type
 *
 * @{
 */
#define CS47L15_BUS_TYPE_I2C                            (0)
#define CS47L15_BUS_TYPE_SPI                            (1)
/** @} */

/**
 * @defgroup CS47L15_STATE_
 * @brief State of the driver
 *
 * @see cs47l15_t member state
 *
 * @{
 */
#define CS47L15_STATE_UNCONFIGURED                      (0)
#define CS47L15_STATE_CONFIGURED                        (1)
#define CS47L15_STATE_STANDBY                           (2)
#define CS47L15_STATE_ERROR                             (4)
 /** @} */

/**
 * @defgroup CS47L15_MODE_
 * @brief Mode of the driver
 *
 * @see cs47l15_t member mode
 *
 * @{
 */
#define CS47L15_MODE_HANDLING_CONTROLS                  (0)
#define CS47L15_MODE_HANDLING_EVENTS                    (1)
/** @} */
 ...
 */
#define CS47L15_CP_READ_BUFFER_LENGTH_BYTES             (4)

/**
 * @defgroup CS47L15_POWER_
 ...
 * @{
 */
#define CS47L15_POWER_UP                                (0)
#define CS47L15_POWER_DOWN                              (1)
#define CS47L15_POWER_MEM_ENA                           (2)
#define CS47L15_POWER_MEM_DIS                           (3)
/** @} */

/**
 * @defgroup CS47L15_EVENT_FLAG_
 * @brief Flags passed to Notification Callback to notify BSP of specific driver events
 *
 * @see CS47L15_notification_callback_t argument event_flags
 *
 * @{
 */
#define CS47L15_EVENT_FLAG_BOOT_DONE                    (1 << 4)
#define CS47L15_EVENT_FLAG_DSP_IRQ1                     (1 << 3)
#define CS47L15_EVENT_FLAG_DSP_BUS_ERROR                (1 << 2)
#define CS47L15_EVENT_FLAG_OVERTEMP_ERROR               (1 << 1)
#define CS47L15_EVENT_FLAG_OVERTEMP_WARNING             (1 << 0)
/** @} */
 ...
 * @return none
 */
typedef void (*cs47l15_notification_callback_t)(uint32_t event_flags, void *arg);
 ...
 * instantiated outside the scope of the driver source and initialized by the cs47l15_initialize public API.
 */
typedef struct
{
    uint32_t state;                                      ///< General driver state - @see CS47L15_STATE_
    uint32_t mode;                                       ///< General driver mode - @see CS47L15_MODE_
    uint32_t devid;                                      ///< CS47L15 DEVID of current device
    uint32_t revid;                                      ///< CS47L15 REVID of current device
    /*
     * List of register address/value pairs to write on wake up from hibernate
     */
    cs47l15_config_t config;                             ///< Driver configuration fields - see cs47l15_config_t
 ...
 *
 * @return
 * - CS47L15_STATUS_FAIL        if pointer to driver is NULL
 ...
 *
 * @return
 * - CS47L15_STATUS_FAIL        if any pointers are NULL
 * - CS47L15_STATUS_OK          otherwise
 *
 */
 ...
 *
 * @return
 * - if in UNCONFIGURED or ERROR state, returns CS47L15_STATUS_OK
 * - else if in HANDLING_CONTROLS mode, returns CS47L15_STATUS_OK
 ...
 * Reset the CS47L15
 *
 * This call performs all necessary reset of the CS47L15 from power-on-reset to being ready to be put into
 * its first use-case.
 * - toggling RESET line
 ...
 * - otherwise, returns CS47L15_STATUS_OK
 *
 */
 ...
 *
 * While cs47l15_write_block loads the actual FW/COEFF data into ADSP2 RAM, cs47l15_boot will finish the boot process
 * by:
 ...
 *
 * @return
 * - CS47L15_STATUS_FAIL        if requested power_state is invalid, or if the call to change power state fails
 * - CS47L15_STATUS_OK          otherwise
 *
 */
