...
uint32_t bsp_dut_boot(void)
{
    ...
    for (i = 0; i < cs40l5x_total_fw_blocks; i++) {
        ret = regmap_write_block((&cs40l5x_driver.config.bsp_config.cp_config),
                                 ...
                                 (uint8_t *)cs40l5x_fw_blocks[i].bytes,
                                 cs40l5x_fw_blocks[i].block_size);
    }
    ...
    for (i = 0; i < cs40l5x_wt_total_coeff_blocks; i++) {
        ret = regmap_write_block((&cs40l5x_driver.config.bsp_config.cp_config),
                                cs40l5x_wt_coeff_blocks[i].address,
                                (uint8_t *)cs40l5x_wt_coeff_blocks[i].bytes,
                                cs40l5x_wt_coeff_blocks[i].block_size);
        if (ret == CS40L5X_STATUS_FAIL)
        {
            return BSP_STATUS_FAIL;
        }
