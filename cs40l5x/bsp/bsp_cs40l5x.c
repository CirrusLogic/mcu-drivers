 ...
 * LOCAL VARIABLES
 **********************************************************************************************************************/
static cs40l5x_t cs40l5x_driver;


static cs40l5x_bsp_config_t bsp_config =
{
    ...
    .cp_config.receive_max = 0, // No calls to regmap_read_block for the cs40l5x driver
};

static cs40l5x_df0_table_entry_t dynamic_f0;
...
uint32_t bsp_dut_initialize(void)
{
    ...
    cs40l5x_config_t haptic_config;

    memset(&haptic_config, 0, sizeof(cs40l5x_config_t));

    // Initialize chip drivers
    haptic_status = cs40l5x_initialize(&cs40l5x_driver);
    if (haptic_status == CS40L5X_STATUS_OK)
    {
        haptic_config.bsp_config = bsp_config;

        haptic_config.syscfg_regs = cs40l5x_syscfg_regs;
        haptic_config.syscfg_regs_total = CS40L5X_SYSCFG_REGS_TOTAL;

        haptic_config.is_ext_bst = true;

        haptic_config.dynamic_f0_threshold = 0x20C5;

        haptic_status = cs40l5x_configure(&cs40l5x_driver, &haptic_config);
...
uint32_t bsp_dut_reset(void)
{
    ...
    ret = cs40l5x_reset(&cs40l5x_driver);
    ...
    ret = cs40l5x_timeout_ticks_set(&cs40l5x_driver, 500);
...
uint32_t bsp_dut_get_gpio_input_level(unsigned int gpio)
{
    ...
    regmap_read(&cs40l5x_driver.config.bsp_config.cp_config,
        CS40L5X_GPIO_STATUS1, &gpio_status);
...
uint32_t bsp_dut_boot(void)
{
    ...
    // fw_info pointer to cs40l5x_boot
    if (boot_state.fw_info.header.fw_version != CS40L5X_WT_ONLY)
    {
        ret = cs40l5x_boot(&cs40l5x_driver, NULL);
        if (ret != CS40L5X_STATUS_OK)
        {
            return ret;
        }
    ...
    if ((boot_state.fw_info.header.fw_version < CS40L5X_MIN_FW_VERSION) && (boot_state.fw_info.header.fw_version != CS40L5X_WT_ONLY))
    {
        return BSP_STATUS_FAIL;
    }
    ...
    if (boot_state.fw_info.header.fw_version != CS40L5X_WT_ONLY)
    {
        ret = cs40l5x_boot(&cs40l5x_driver, &boot_state.fw_info);
    }
...
uint32_t bsp_dut_calibrate(void)
{
    ...
    ret = cs40l5x_calibrate(&cs40l5x_driver);
...
uint32_t bsp_dut_timeout_ticks_set(uint32_t ms)
{
    ...
    ret = cs40l5x_timeout_ticks_set(&cs40l5x_driver, ms);
    if (ret != CS40L5X_STATUS_OK)
    {
        return BSP_STATUS_FAIL;
    }
...
uint32_t bsp_dut_hibernate(void)
{
    ...
    ret = cs40l5x_power(&cs40l5x_driver, CS40L5X_POWER_HIBERNATE);
    if (ret != CS40L5X_STATUS_OK)
    {
        return BSP_STATUS_FAIL;
    }
...
uint32_t bsp_dut_wake(void)
{
    ...
    ret = cs40l5x_power(&cs40l5x_driver, CS40L5X_POWER_WAKE);
    if (ret != CS40L5X_STATUS_OK)
    {
      return BSP_STATUS_FAIL;
    }
...
uint32_t bsp_dut_set_click_compensation(bool f0_enable, bool redc_enable)
{
    ...
    ret = cs40l5x_set_click_compensation_enable(&cs40l5x_driver, f0_enable, redc_enable);
...

{
    ...
    ret = cs40l5x_set_broadcast_enable(&cs40l5x_driver, enable);
...
uint32_t bsp_dut_set_redc(uint32_t redc)
{
    ...
    ret = cs40l5x_set_redc(&cs40l5x_driver, redc);
    if (ret != CS40L5X_STATUS_OK)
    {
        return BSP_STATUS_FAIL;
    }
...
uint32_t bsp_dut_set_f0(uint32_t f0)
{
    ...
    ret = cs40l5x_set_f0(&cs40l5x_driver, f0);
    if (ret != CS40L5X_STATUS_OK)
    {
        return BSP_STATUS_FAIL;
    }
...
uint32_t bsp_dut_dynamic_f0_set_enable(bool enable)
{
    ...
    // Enable Dynamic F0
    ret = cs40l5x_set_dynamic_f0(&cs40l5x_driver, enable);
...
uint32_t bsp_dut_disable_gpio_triggers(void)
{
    ...
    for (i = 0; i < 16; i++) {
        ret = regmap_write(&cs40l5x_driver.config.bsp_config.cp_config,
                CS40L5X_GPIO_HANDLERS_BASE + i*4, 0x1FF);
        if (ret != CS40L5X_STATUS_OK)
        {
            return BSP_STATUS_FAIL;
        }
...
uint32_t bsp_dut_configure_gpio_input(unsigned int gpio)
{
    ...
    return regmap_write(&cs40l5x_driver.config.bsp_config.cp_config,
        CS40L5X_GPIO_CTRL1 + (4 * (gpio - 1)),
                                        ...
                                        uint8_t attenuation, bool ram, uint8_t plybck_index)
{
    ...
    ret = cs40l5x_configure_gpio_trigger(&cs40l5x_driver, gpio, rth, attenuation, ram, plybck_index);
...
uint32_t bsp_dut_trigger_rth_pwle(bool is_simple, rth_pwle_section_t **pwle_data, uint8_t num_sections, uint8_t repeat)
{
    ...
    if (is_simple)
    {
        ret = cs40l5x_trigger_pwle(&cs40l5x_driver, pwle_data);
    }
    else
    {
        ret = cs40l5x_trigger_pwle_advanced(&cs40l5x_driver, pwle_data, repeat, num_sections);
    }
...
uint32_t bsp_dut_trigger_rth_pcm(uint8_t *pcm_data, uint32_t num_sections, uint16_t buffer, uint16_t f0, uint16_t redc)
{
    ...
    ret = cs40l5x_trigger_pcm(&cs40l5x_driver, pcm_data, num_sections, buffer, f0, redc);
...
uint32_t bsp_dut_dynamic_calibrate(uint8_t index)
{
    ...
    ret = cs40l5x_get_dynamic_f0(&cs40l5x_driver, &dynamic_f0);
    if (ret != CS40L5X_STATUS_OK)
    {
        return BSP_STATUS_FAIL;
    }
