 ...
 *
 * @stringp: Pointer to string pointer; updated to point past the delimiter
 * @delim: Delimiter string
 *
 * Locates the first occurrence of any character in delimiter string within input string,
 * replaces it with '\0', and advances string pointer past it.
 *
 * Returns the original value of *@stringp, or NULL if *@stringp was NULL.
 *
 */
...
/*
 * wt_type10_comp_to_buffer() - Format data to be written to OWT with driver API functions
 *
 * @dev: Pointer to driver struct
 * @comp: Pointer to composite struct to be written to OWT
 *
 * Copies over wt_type10_comp data to the driver's composite OWT functions and composite section
 * data structure
 */

static int wt_type10_comp_to_buffer(const struct device *dev, struct wt_type10_comp *comp)
{
    int i;
    int ret = BSP_STATUS_OK;

    ret = bsp_cs40l5x_write_owt_composite_header(dev, comp->nsections, comp->repeat);
    if (ret) {
        return ret;
    }
    for (i = 0; i < comp->nsections; i++) {
        ...
        section.amplitude = comp->sections[i].wvfrm.amplitude;
        section.waveform_idx = comp->sections[i].wvfrm.index;
        section.nested_repeats = comp->sections[i].repeat;
        section.delay = comp->sections[i].delay;

        section.rom_subwave =
            (comp->sections[i].flags & WT_TYPE10_ROM_SUBWAVE_FLAG) ? 1 : 0;
        section.owt_subwave =
            (comp->sections[i].flags & WT_TYPE10_OWT_SUBWAVE_FLAG) ? 1 : 0;
        section.duration_present =
            (comp->sections[i].flags & WT_TYPE10_COMP_DURATION_FLAG) ? 1 : 0;

        if (section.duration_present) {
            section.duration = comp->sections[i].wvfrm.duration;
        }
        ret = bsp_cs40l5x_write_owt_composite_section(dev, section);
        if (ret) {
            return ret;
        }
    }
    ret = bsp_cs40l5x_push_owt_composite(dev);
    return ret;
}
 ...
 *
 */
static int wt_type10_comp_waveform_get(char *str, struct wt_type10_comp_section *section)
{
...
static int wt_type10_comp_decode(struct wt_type10_comp *comp,
                 enum wt_type10_comp_specifier specifier, char *str)
{
 ...
 *
 * @dev: Pointer to the driver struct
 ...
 *
 * Returns BSP_STATUS_OK upon success
 * Returns negative errno in error case.
 *
 */

static int wt_type10_comp_str_to_bin(const struct device *dev, char *full_str)
{
    ...
    if (comp.sections[comp.nsections].wvfrm.amplitude || comp.sections[comp.nsections].delay) {
        comp.nsections++;
    }

    return wt_type10_comp_to_buffer(dev, &comp);
}
 ...
 *
 */
static int wt_type12_pwle_feature_entry(struct wt_type12_pwle *pwle, char *token)
{
    int val = atoi(token);
 ...
 *
 */
static int wt_type12_pwle_wait_time_entry(struct wt_type12_pwle *pwle, char *token)
{
    int ret, val;
                     ...
                     char *phase_offset, struct wt_type12_pwle_section *section)
{
    ...
    if (offset == 1 && (pwle->feature & WT_TYPE12_PWLE_LF0T_FLAG) == 0) {
        printf("Live F0 tracking must be enabled in the feature bitmap to use phase "
               "offset\n");
        return -EINVAL;
    }
 ...
 *
 */
static int wt_type12_pwle_vb_target_entry(struct wt_type12_pwle *pwle, char *token,
                      struct wt_type12_pwle_section *section)
{
    int ret, val;
 ...
 *
 * @dev: Pointer to driver struct
 * @pwle: Pointer to pwle struct to be written to OWT
 *
 * Copies over wt_type12_pwle data to the driver's pwle OWT functions and pwle section data
 * structure
 *
 * Returns BSP_STATUS_OK on success
 * Returns negative errno/BSP_STATUS_FAIL in error case.
 */
static int wt_type12_pwle_write(const struct device *dev, struct wt_type12_pwle *pwle)
{
    // TODO: Add initial section back in once cs40l5x pwle OWT API has full metadata support,
    // must also switch from dspmem function to driver OWT functions

    // uint32_t header_words = WT_TYPE12_HEADER_WORDS;
    int i;
    int ret = BSP_STATUS_OK;

    // pwle->feature &= (~WT_TYPE12_PWLE_METADATA_FLAG);
    ...
    uint32_t wf_length = pwle->wlength;
    uint8_t repeats = pwle->repeat;
    uint16_t wait_time = pwle->wait;
    uint8_t num_sections = pwle->nsections;

    ret = bsp_cs40l5x_write_owt_pwle_header(dev, &next_first_byte, wf_length, repeats,
                        wait_time, num_sections);
    if (ret) {
        return ret;
    }
    ...
    for (i = 0; i < pwle->nsections; i++) {
        ...
        section.time = pwle->sections[i].time;
        section.level = pwle->sections[i].level;
        section.frequency = pwle->sections[i].frequency;

        section.chirp_mode = (pwle->sections[i].flags & WT_TYPE12_PWLE_CHIRP_BIT) ? 1 : 0;
        section.half_cycle_def =
            (pwle->sections[i].flags & WT_TYPE12_PWLE_HALF_CYCLE_BIT) ? 1 : 0;
        section.ext_frequency =
            (pwle->sections[i].flags & WT_TYPE12_PWLE_EXT_FREQ_BIT) ? 1 : 0;
        section.F0_relative_freq =
            (pwle->sections[i].flags & WT_TYPE12_PWLE_REL_FREQ_BIT) ? 1 : 0;
        section.phase_offset =
            (pwle->sections[i].flags & WT_TYPE12_PWLE_PHASE_OFFSET_BIT) ? 1 : 0;

        if (i == 0) {
            ret = bsp_cs40l5x_write_owt_pwle_section(dev, true, &next_first_byte,
                                 section);
            if (ret) {
                return ret;
            }
        } else {
            ret = bsp_cs40l5x_write_owt_pwle_section(dev, false, &next_first_byte,
                                 section);
            if (ret) {
                return ret;
            }
    ...
    ret = bsp_cs40l5x_push_owt_pwle(dev, &next_first_byte);
    return ret;
}
 ...
 *
 */
static int wt_type12_pwle_str_to_bin(const struct device *dev, char *full_str)
{
    ...
    char *str, *freq_val = NULL, *phase_offset = NULL;
    ...
    while (str != NULL) {
        ...
        case PWLE_SPEC_TIME:
            ...
            ret = wt_type12_pwle_time_entry(&pwle, str, section, &indef);
            if (ret) {
                return ret;
            }
        ...
        case PWLE_SPEC_RELFREQ:
            if (!freq_val || !phase_offset) {
                printf("Malformed PWLE, missing freq or phase offset before "
                       "relfreq\n");
                return -EINVAL;
            }

            ret = wt_type12_pwle_freq_entry(&pwle, str, freq_val, phase_offset,
                            section);
            if (ret) {
                return ret;
            }
        ...
        case PWLE_SPEC_VBT:
            if (section->flags & WT_TYPE12_PWLE_AMP_REG_BIT) {
                ret = wt_type12_pwle_vb_target_entry(&pwle, str, section);
                if (ret) {
                    return ret;
                }
            }
    ...
    return wt_type12_pwle_write(dev, &pwle);
}
...
int get_owt_data(const struct device *dev, char *full_str)
{
    ...
    if (full_str[0] == 'S') {
        ret = wt_type12_pwle_str_to_bin(dev, full_str);
    } else {
        ret = wt_type10_comp_str_to_bin(dev, full_str);
    }
