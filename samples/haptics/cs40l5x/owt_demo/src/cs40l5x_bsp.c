...
int cs40l5x_i2c_write_bus(const struct i2c_dt_spec *spec, const uint32_t reg_addr, uint32_t val,
              uint16_t bus_addr)
{
...
static uint32_t cs40l5x_register_gpio_cb(const struct gpio_dt_spec *gpio_id, bsp_callback_t cb,
                     void *cb_arg)
{
    // TODO
    return 0;
...
bsp_driver_if_t cs40l5x_bsp_driver_if = {.set_timer = cs40l5x_set_timer,
...
int bsp_cs40l5x_delete_owt(const struct device *dev, int owt_idx)
{
    ...
    if (state == CS40L5X_DSP_STATE_ACTIVE) {
        ret = regmap_write(&config->i2c, CS40L5X_DSP_VIRTUAL1_MBOX_1,
                   CS40L5X_DSP_MBOX_STOP_PLAYBACK);
        if (ret) {
            return ret;
        }
        ret = regmap_poll_reg(&config->i2c, PM_PM_CUR_STATE, CS40L5X_DSP_STATE_STANDBY, 10,
                      10);
        if (ret) {
...
int bsp_cs40l5x_write_owt_pcm(const struct device *dev, uint8_t sample_rate, const uint8_t *buffer,
                  uint32_t buffer_size)
{
    ...
    ret = cs40l5x_write_owt_pcm(drv, sample_rate, buffer, buffer_size);
...
int bsp_cs40l5x_write_owt_composite_header(const struct device *dev, uint8_t num_waveforms,
                       uint8_t repeats)
{
    struct cs40l5x_bsp *data = dev->data;
    cs40l5x_t *drv = &data->priv;
                        ...
                        struct cs40l5x_owt_composite_section_params section)
{
    ...
    ret = cs40l5x_write_owt_composite_section(drv, section.nested_repeats, section.waveform_idx,
...
int bsp_cs40l5x_push_owt_composite(const struct device *dev)
{
    struct cs40l5x_bsp *data = dev->data;
    cs40l5x_t *drv = &data->priv;
    uint32_t ret;
                       ...
                       struct cs40l5x_owt_pwle_section_params section)
{
    ...
    if (first_section) {
        ret = cs40l5x_write_owt_pwle_section(drv, first_section, next_first_byte,
                             TIME_DEFAULT, section.level, FREQ_DEFAULT, 0,
                             0, 0, 0, 0, 0);

    } else {
        ret = cs40l5x_write_owt_pwle_section(
...
int bsp_cs40l5x_push_owt_pwle(const struct device *dev, uint8_t *next_first_byte)
{
    struct cs40l5x_bsp *data = dev->data;
    cs40l5x_t *drv = &data->priv;
    uint32_t ret;

    ret = cs40l5x_push_owt_pwle(drv, next_first_byte);
    if (ret) {
        LOG_ERR("Error pushing owt waveform");
        return ret;
    }
