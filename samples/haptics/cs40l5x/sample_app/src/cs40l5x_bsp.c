...
static int cs40l5x_write_fw_blocks(struct i2c_dt_spec *i2c, const halo_boot_block_t *blocks,
                   int num_blocks)
{
    int i, ret;
    ...
    for (i = 0; i < num_blocks; i++) {
        ...
        ret = cs40l5x_i2c_write_bulk_dt(i2c, address, (const uint8_t *)buffer, bytes);
        if (ret != 0) {
            return ret;
        }
        k_msleep(5);
...
static int cs40l5x_init(const struct device *dev)
{
    ...
    uint32_t val;
    int ret;
    LOG_INF("cs40l5x_init for device %s\n", dev->name);
...
// static void cs40l5x_isr(void *arg)
// {
// }
