...
static int cmd_store_pcm_owt(const struct shell *sh, size_t argc, char **argv)
{
    ...
    switch (fs) {
    case (8000):
        sample_rate = CS40L50_OWT_SAMPLE_RATE_8K;
        break;
    case (4000):
        sample_rate = CS40L50_OWT_SAMPLE_RATE_4K;
        break;
    case (24000):
        sample_rate = CS40L50_OWT_SAMPLE_RATE_24K;
        break;
    case (48000):
        sample_rate = CS40L50_OWT_SAMPLE_RATE_48K;
        break;
    default:
    ...
    ret = bsp_cs40l50_write_owt_pcm(cs40l50, sample_rate, buffer, buf_size);
    free(buffer);
    if (ret) {
...
static int cmd_store_composite_owt(const struct shell *sh, size_t argc, char **argv)
{
    ...
    bsp_cs40l50_write_owt_composite_header(cs40l50, num_waveforms, repeats);
    ...
    for (int i = 0; i < num_waveforms; i++) {
        ...
        bsp_cs40l50_write_owt_composite_section(cs40l50, section);
    }

    bsp_cs40l50_push_owt_composite(cs40l50);
    return 0;
}
...
static int cmd_store_pwle_owt(const struct shell *sh, size_t argc, char **argv)
{
    ...
    bsp_cs40l50_write_owt_pwle_header(cs40l50, &next_first_byte, repeats, wait_time,
                      num_sections);
    ...
    bsp_cs40l50_write_owt_pwle_section(cs40l50, true, &next_first_byte, section);
    ...
    for (int i = 0; i < num_sections - 1; i++) {
        ...
        bsp_cs40l50_write_owt_pwle_section(cs40l50, false, &next_first_byte, section);
    }
    bsp_cs40l50_push_owt_pwle(cs40l50, &next_first_byte);
    return 0;
}
...
static int cmd_trigger_owt(const struct shell *sh, size_t argc, char **argv)
{
    ...
    return haptics_cs40l50_trigger_owt(cs40l50, idx);
}

static int cmd_get_num_owt_effects(const struct shell *sh, size_t argc, char **argv)
{
    uint32_t num;
    uint32_t ret = bsp_cs40l50_get_num_owt_wf(cs40l50, &num);
    if (ret) {
        return ret;
    }
...
SHELL_STATIC_SUBCMD_SET_CREATE(
    cs40l50_cmds,
    SHELL_CMD_ARG(store_pcm_owt, NULL, CS40L50_STORE_PCM, cmd_store_pcm_owt, 4, 0),
    SHELL_CMD_ARG(store_composite_owt, NULL, CS40L50_STORE_COMPOSITE, cmd_store_composite_owt,
              1, 0),
    SHELL_CMD_ARG(store_pwle_owt, NULL, CS40L50_STORE_PWLE, cmd_store_pwle_owt, 1, 0),
    SHELL_CMD_ARG(trigger_owt, NULL, CS40L50_TRIGGER, cmd_trigger_owt, 2, 0),
    SHELL_CMD_ARG(get_num_owt_effects, NULL, CS40L50_GET_NUM_OWT_EFFECTS,
...
int main(void)
{
    ...
    struct cs40l50_haptic_source_config hap_cfg = {
        .index = CS40L50_HAPTIC_ROM_CLICK_1_VCM,
        .bank = ROM_BANK,
    };
