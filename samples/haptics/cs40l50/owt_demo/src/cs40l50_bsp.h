...
struct cs40l50_owt_composite_section_params {
...
struct cs40l50_owt_pwle_section_params {
    uint16_t time;
    uint16_t level;
    uint16_t frequency;
    uint8_t chirp_mode;
 ...
 * cs40l50_write_owt_composite_header()
 * - BSP_STATUS_OK          otherwise
 */
 ...
 *
 * This function should be called N times following the call to bsp_cs40l50_write_owt_header
 ...
 * cs40l50_write_owt_composite_section()
 * - BSP_STATUS_OK          otherwise
 */
...
/**
 * Push the most recently written OWT compositre effect to the OWT based on the number of sections
 * written and the next available OWT index
 *
 * @param [in] dev              Pointer to the driver state
 *
 * @return
 * - BSP_STATUS_FAIL        If failure to call cs40l50_push_owt_composite
 * - BSP_STATUS_OK          otherwise
 */
