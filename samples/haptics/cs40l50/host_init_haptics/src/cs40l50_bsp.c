...
int bsp_cs40l50_write_owt_section(const struct device *dev,
...
int bsp_cs40l50_write_owt_composite_one_section(const struct device *dev,
                        struct cs40l50_owt_section_params section)
{
    uint32_t ret;
    struct cs40l50_bsp *data = dev->data;
    ...
    ret = cs40l50_write_owt_composite_one_section(
        drv, section.nested_repeats, section.waveform_idx, section.amplitude, section.delay,
        section.owt_subwave, section.rom_subwave);
    if (ret) {
...
int bsp_cs40l50_host_initiated_trigger(const struct device *dev, HIH_effect effect,
                       ...
                       uint32_t retrigger_period, uint32_t cutoff_time)
{
    ...
    if (effect.label == EFFECT_NAME) {
        ret = bsp_cs40l50_convert_effect_name(effect.HIH_effect_identifier.effectName,
                              &idx);
        if (ret) {
    ...
    struct cs40l50_owt_section_params section = {.nested_repeats = repeats,
                             .waveform_idx = idx,
                             .amplitude = intensity,
