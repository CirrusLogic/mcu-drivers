...
#define CS40L50_EFFECT_MSFT_ID_TRIGGER                                                             \
    SHELL_HELP("Play Effect indicated by Microsoft ID with Given Intensity, Repeat Count, "    \
           "Retrigger Period (ms), and max waveform playback time (ms)",                   \
           "<msft_ID> <intensity> <repeats> <retrigger_period> <cutoff_time>")
#define CS40L50_EFFECT_NAME_TRIGGER                                                                \
    SHELL_HELP("Play Effect indicated by effect name with Given Intensity, Repeat Count, and " \
           "Retrigger Period (ms), and max waveform playback time (ms)",                   \
           "<effect_name> <intensity> <repeats> <retrigger_period> <cutoff_time>")
...
static int cmd_effect_name_trigger(const struct shell *sh, size_t argc, char **argv)
{
    ...
    ret = bsp_cs40l50_host_initiated_trigger(cs40l50, effect_data, amplitude, nested_repeats,
                         delay, cutoff_time);
    return ret;
}
...
SHELL_STATIC_SUBCMD_SET_CREATE(cs40l50_cmds,
                   SHELL_CMD_ARG(list_wt, NULL, CS40L50_LIST_WT, cmd_list_wt, 1, 0),
                   SHELL_CMD_ARG(effect_msft_id_trigger, NULL,
                         CS40L50_EFFECT_MSFT_ID_TRIGGER,
                         cmd_effect_msft_id_trigger, 6, 0),
                   SHELL_CMD_ARG(effect_name_trigger, NULL, CS40L50_EFFECT_NAME_TRIGGER,
                         cmd_effect_name_trigger, 6, 0),
