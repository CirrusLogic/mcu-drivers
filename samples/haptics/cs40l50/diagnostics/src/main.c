...
int main(void)
{
    ...
    while (1) {
        if (gpio_pin_get_dt(&button)) {
            // Set error flag to omit VDD amp low, VDD amp high, ReDC low, and ReDC high
            // errors with sample_mask Set sample_mask to 0 to show all diagnostic
            // errors
            uint32_t sample_mask = CS40L50_DIAG_VDD_AMP_HI | CS40L50_DIAG_VDD_B_HI |
                           CS40L50_DIAG_ReDC_HI | CS40L50_DIAG_LE_HI;
            ...
            ret = haptics_cs40l50_diagnostics(dev1, &diagnostics_data, sample_mask,
                              &diagnostics_err);
            if (ret) {
