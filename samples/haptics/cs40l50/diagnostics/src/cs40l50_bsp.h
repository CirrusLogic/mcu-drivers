...
typedef struct {
     ...
     *
     */
    uint32_t (*register_gpio_cb)(const struct gpio_dt_spec *gpio, bsp_callback_t cb,
                     void *cb_arg);
 ...
 * Perform Diagnostics
 *
 * This function acts as a wrapper for the cs40l50 diagnostics function, decoding and printing any
 * detected diagnostic errors. A diagnostic error mask is taken in and used to calculate a single
 * diagnostic output flag with masked tests omitted.
 *
 * @param [in]  dev             data structure containing necessary device information
 * @param [out] diag_data       bitfield output of diagnostics containing all diagnostics errors
 * detected
 * @param [in]  mask            bitmask of diagnostic error flags to omit from diag_flag
 * @param [out] diag_flag       boolean output flag that is set when any non-masked diagnostic
 * errors are detected
 *
 * @return
